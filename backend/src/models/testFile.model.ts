...
export interface ITestFile extends Document {
  ...
  testCode: string; // The actual generated test code
  summary: ITestSummary;
  validation: IValidation;
  metadata: ITestMetadata;
  status: 'draft' | 'generated' | 'saved' | 'applied'; // Lifecycle status
...
export interface ITestFileModel extends Model<ITestFile> {
  ...
  findByRepository(repositoryId: string, userId: Types.ObjectId): Promise<ITestFile[]>;
...
const TestFileSchema = new Schema<ITestFile>(
  {
    ...
    status: {
      type: String,
      enum: ['draft', 'generated', 'saved', 'applied'],
      default: 'generated'
    },
...
TestFileSchema.methods.getValidationSummary = function(): { 
  ...
  const score = Math.max(0, 100 - (syntaxIssues * 10) - (logicIssues * 5) - (coverageGaps * 3));
...
TestFileSchema.statics.findBySession = function(sessionId: Types.ObjectId) {
  return this.find({ sessionId, isActive: true }).sort({ 'metadata.generatedAt': -1 });
};

// Static method to find tests by repository
TestFileSchema.statics.findByRepository = function(repositoryId: string, userId: Types.ObjectId) {
  return this.find({ repositoryId, userId, isActive: true }).sort({ 'metadata.generatedAt': -1 });
};
...
export const TestFile = model<ITestFile, ITestFileModel>("TestFile", TestFileSchema, "test_files");

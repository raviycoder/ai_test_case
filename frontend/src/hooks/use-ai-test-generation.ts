...
interface UseAITestGenerationReturn {
  ...
  startGeneration: (
    request: Omit<TestGenerationRequest, "sessionId">
  ) => Promise<void>;
  ...
  reset: () => void;
  deleteTestFile: (sessionId: string, filePath: string) => Promise<{ success: boolean; message: string }>;
}

export const useAITestGeneration = (
  options: UseAITestGenerationOptions = {}
): UseAITestGenerationReturn => {
  ...
  const startGenerationMutation = useMutation({
    mutationFn: async (request: Omit<TestGenerationRequest, "sessionId">) => {
      ...
      const fullRequest: TestGenerationRequest = {
        ...request,
  ...
  const startGeneration = useCallback(
    async (request: Omit<TestGenerationRequest, "sessionId">) => {
  ...
  const getTestFileByPath = useCallback(
    async (filePath: string, providedSessionId?: string) => {
      try {
        setIsLoading(true);
        setError(null);
        ...
        if (dto) {
          setTestFile({
            ...dto.data,
            testCode: convertToText(
              dto.data.testCode as unknown as { data: number[] }
            ),
          } as unknown as TestFileDto);
          return dto;
        } else {
          setTestFile(null);
          return null;
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to get test file";
        setError(errorMessage);
        return null;
      } finally {
        setIsLoading(false);
  ...
  // get Test File Paths
  const getTestFilePaths = useCallback(
    async (sessionId: string, repositoryId: string) => {
      try {
        setIsLoading(true);
        setError(null);
  ...
  const deleteTestFile = useCallback(
    async (sessionId: string, filePath: string) => {
      setIsLoading(true);
      setError(null);
      const result = await deleteTestFileApi(sessionId, filePath);
      if (result.success) {
        // Handle successful deletion (e.g., update state)
        setTestFile(null);
        setGeneratedTests((prev) =>
          prev.filter((test) => test.filePath !== filePath)
        );
        // Optionally refresh test file paths
        if (session) {
          getTestFilePaths(session.sessionId, session.repositoryId as string);
        }
      } else {
